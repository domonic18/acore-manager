...
export function registerManifestTool(): void {
  registerTool({
    ...
    description:
      '读取 COS 日志清单：当日已上传的日志文件构成、大小、md5、行数，并校验 worldserver/authserver/anticheat/crash 四类是否到齐。文件不存在返回 present=false（断传或未到上传时间）。realm 未传时用系统默认 realm。',
    schema: z.object({
    ...
    handler: async (args) => {
      const { date } = args as { date: string; realm?: string };
