...
| 3 | **游戏库只读，acm 库可写** | TypeORM 游戏三库连接保持 SELECT-only；新增 acm 庂数据源承载报告 / 会话 / 配置 / 审计 |
| 4 | **全链路可回放** | 每次工具调用（参数 / 耗时 / 行数）写入 ai_tool_audit；对话正本落 MySQL |
| 5 | **失败可降级** | SSE→非流式、DB 聚合缺表→日志为源、Redis 重启→会话作废，均不阻塞核心链路 |
...
┌─────────────────────────────────────────▼──────────────────────────────┐
│ ③ SCF Job 函数（定时触发器 06:00，ACM_ENTRY_MODE=job，一次性执行后退出）      │
│   inspection.service → Agent 运行时（与 Web 函数同一镜像 / 同一份代码包）     │
│   全 agentic 日志分析 → 白名单工具 → 报告 JSON+MD                          │
└───────────────┬──────────────┬───────────────┬────────────────────────┘
                │ ④白名单只读查询  │ ⑤报告落库       │ ⑥日报/告警
                ▼               ▼                ▼
     ┌─────────────────┐  ┌────────────┐  ┌────────────┐
     │ AzerothCore MySQL│  │ acm 库      │  │ 飞书机器人   │
     │ auth/characters/ │  │ (可写)      │  └────────────┘
     │ world (仅 SELECT)│  │ 6+1 张表    │
     └─────────────────┘  └─────▲──────┘
                                │ ⑦API
┌───────────────────────────────┴────────────────────────────────────────┐
│ ⑧ SCF Web 函数（ACM 单体镜像，ACM_ENTRY_MODE=web）                        │
│  Express :9000                                                          │
...
└──────────────┬───────────────┬───────────────────┬─────────────────────┘
               │               │                   │
        ┌──────┴──────┐  ┌─────┴──────┐   ┌────────┴────────┐
        │ Redis        │  │ GLM API    │   │ ACM 前端          │
        │ (checkpoint) │  │ (OpenAI 兼容)│   │ 助手对话 / 诊断报告 │
        └──────────────┘  └────────────┘   └─────────────────┘
```

### 2.2 与现有架构的关系

- **进程内扩展，不新增常驻服务**：Agent 运行时是 Express 进程内的一个模块族（`backend/src/agent/`），复用现有 JWT 鉴权、GM 等级守卫、pino 日志、TypeORM 连接管理与 ioredis
- **镜像复用**：仍为前后端合一的单镜像；Job 形态通过入口脚本按 `ACM_ENTRY_MODE` 环境变量切换（`web` 启动 Express / `job` 执行一次性巡检后退出）
- **新增外部依赖仅两类**：COS（已有腾讯云生态）与 GLM OpenAI 兼容 API；Redis / MySQL 复用现有实例
...
```
backend/src/
├── agent/                          # AI Agent 模块族（新增）
│   ├── runtime/
│   │   ├── agent-factory.ts        # createDeepAgent 组装 + 指纹 LRU 缓存
│   │   ├── checkpointer.ts         # Redis checkpointer 单例
...
```
createDeepAgent({
  model:        ChatOpenAI({ configuration.baseURL: cfg.baseUrl, ... })  // GLM OpenAI 兼容
...
| 存储 | 内容 | 说明 |
|------|------|------|
| Redis（`@langchain/langgraph-checkpoint-redis`） | LangGraph 线程状态（消息、todo、中间状态） | thread_id = 会话 ID；不开启 AOF（已决策），重启后会话作废为可接受降级；key 设 TTL 自动清理（JS 版 TTL 能力开发前核实，不支持则由服务层显式删除） |
| MySQL acm 库（ai_chat_session / ai_chat_message） | 对话正本 | 每轮 assistant 回复完成后异步写入；会话列表 / 历史查询走 MySQL；Redis 丢失后可据正本重建新会话上下文（首版不做重建，重开即可） |
...
新建数据源 `acm`（MySQL 现有实例），TypeORM 实体与表如下；建表由应用启动 synchronize 或迁移脚本负责（遵循 `docs/standard/数据库操作规范.md`）。

### 4.1 ai_model_config（模型出口配置）

| 字段 | 类型 | 说明 |
|------|------|------|
| id | PK | |
| name | varchar | 配置名（如 glm-flash） |
| protocol | varchar | 固定 openai-compatible（预留） |
| base_url / model_name | varchar | 出口地址与模型 |
| api_key_enc | varchar | AES 加密存储，界面不回显明文 |
| temperature / max_tokens | decimal / int | 采样参数 |
| is_default / enabled | boolean | 巡检与对话的默认出口 |
| created_by / updated_at | | gmlevel=4 可改 |
...
- session：id、user_id、thread_id（= 会话 id，对应 Redis checkpoint thread）、title（首条消息截断）、realm、created_at、last_active_at
...
| content_markdown | MEDIUMTEXT | Markdown 全文（详情页查看 + 一键复制） |
| gm_remark | TEXT | GM 处置备注（PUT 修改，gmlevel=4；AI 内容字段不可改） |
...
| conclusion_markdown | MEDIUMTEXT | Markdown 全文（论坛回复复制用） |
...
| DELETE | /api/ai/assistant/sessions/:id | gmlevel≥2 | 删除会话（连带清理 Redis checkpoint key） |
...
| PUT | /api/ai/analysis/targeted/:id | gmlevel=4 | 更新处置备注 / 润色 conclusion_markdown（审计） |
| DELETE | /api/ai/analysis/targeted/:id | gmlevel=4 | 删除定向分析记录（二次确认，审计） |
| PUT | /api/ai/diagnosis/reports/:realm/:date | gmlevel=4 | 更新 GM 处置备注（AI 内容不可改） |
| DELETE | /api/ai/diagnosis/reports/:realm/:date | gmlevel=4 | 删除报告（Redis 摘要失效；COS 归档保留；审计） |
| POST | /api/ai/diagnosis/inspection/trigger | gmlevel=4 | 手动触发指定日期巡检 |
...
| GET/PUT | /api/ai/model-config | gmlevel=4 | 模型出口配置 |
| GET | /api/ai/token-usage?from&to | gmlevel=4 | Token 用量报表 |
...
└── ai-diagnosis/
    ...
    ├── components/ReportManageActions.tsx     # 处置备注编辑 / 删除（二次确认输入文字，gmlevel=4）
...
- **报告管理**：列表行"删除"与详情页"处置备注"编辑仅 gmlevel=4 可见；删除二次确认要求输入确认文字；定向分析结果页 Markdown 可编辑保存后再复制（conclusion_json 不可改）；写操作全量经审计中间件落 GM 操作日志
- **可疑玩家表**：`falsePositiveSignals` 非空时行首展示"疑似误报"徽标；`suggestedAction=ban` 且无信号时才高亮封禁按钮
- **权限映射**：对话 / 报告查看 gmlevel≥2；触发巡检 / 模型配置 / Token 报表 gmlevel=4（路由守卫 + 菜单隐藏 + 按钮 usePermission 三层一致）
...
| 形态 | 触发 | 入口 | 生命周期 |
|------|------|------|---------|
| Web 函数 | HTTPS | `ACM_ENTRY_MODE=web` → `node dist/server.js` | 常驻，承载 API + 静态资源 + SSE 对话 |
| Job 函数 | 定时触发器（06:00）+ 手动触发端点 | `ACM_ENTRY_MODE=job` → `node dist/jobs/inspection-job.js` | 一次性：执行巡检 → 退出 |

入口脚本（docker-entrypoint）按环境变量分流；镜像不变，TCR 同一 tag 复用。

### 7.2 资源与环境变量增量

```bash
# 部署增量环境变量
ACM_ENTRY_MODE=web            # web | job
...
```
GM 浏览器                Web 函数(Express)                GLM / Redis / MySQL
   │  POST /chat (SSE)        │                                │
   ├─────────────────────────▶│ 鉴权+gmlevel 校验                │
...
| Redis 重启（无 AOF） | 进行中会话 checkpoint 丢失 | 旧会话不可续聊，重开即可；MySQL 正本可查 |
| GLM 调用失败 | 重试 3 次退避 → 飞书告警 | 当日报告缺失，历史可查 |
